...
contract Customer {
    ...
    function Customer(CustomerType _customerType, string _name, string _idString){
        owner = msg.sender;
    ...
    function addRecord(string _reason, string _additionReason, string _recordDataURI, bytes32 _recordDataHash, int64 _ratingChange ) returns (uint256 newRecordID, uint256 historyRecordID){
    ...
    function modifyRecord(uint256 _recordID, string _reason, string _modificationReason, string _recordDataURI, bytes32 _recordDataHash, int64 _ratingChange ) returns (uint256 newRecordID, uint256 historyRecordID){
        
        DataRecord memory existingRecord = records[_recordID];
        if (existingRecord.internalID != _recordID) throw;
    ...
    function deleteRecord(uint256 _recordID, string _deletionReason, string _recordDataURI, bytes32 _recordDataHash) returns (uint256 historyRecordID){
    ...
    function getRecord(uint256 recordID) constant returns (        
        uint256 internalID,
        string reason,
        bytes32 reasonHash,
        string recordDataURI,
        ...
        address publishedBy)
        {
            ...
            if (rec.internalID != recordID) throw;
    ...
    function getHistoryRecord(uint256 historyRecordID) constant returns(
        uint256 internalID,
        RecordChangeType changeType,
        address modifiedBy,
        uint modificationTime,
        string reason)
        {
            ...
            if (rec.internalID != historyRecordID) throw;
            internalID = rec.internalID;
    ...
    function getOldRecordInHistoryRecord(uint256 historyRecordID) constant returns(
    ...
    function getNewRecordInHistoryRecord(uint256 historyRecordID) constant returns(
    ...
    function currentRating() constant returns (int64 rating){
        rating =  customerDescription.customerRating;
    }
    
    function maxRecordID() constant returns(uint256 recordID){
        recordID = recordCounter - 1;
    }
    
    function maxHistoryRecordID() constant returns(uint256 recordID){
